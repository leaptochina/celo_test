package com.blueberrysolution.pinelib19.hardware.device
...
object GetDeviceId {
    ...
    //保存文件的路径
    private val CACHE_IMAGE_DIR = "aray/cache/devices"
    //保存的文件 采用隐藏文件的形式进行保存
    ...
    private fun getDevicesDir(context: Context): File {
        ...
        if (Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED) {
            ...
            mCropFile = File(cropdir, DEVICES_FILE_NAME) // 用当前时间给取得的图片命名
        } else {
