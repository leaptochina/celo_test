...
import com.blueberrysolution.pinelib19.pop_up_windows.msgbox.ui.*
...
class MessageBox private constructor(private val context: Context?) : OnClickListener, OnKeyListener,
    ...
    public var dialog: AbsAlertDialog? = null
     ...
     */
    @JvmOverloads
    fun show(message: String, btn: String = A.s(R.string.msgbox_def_btn)) {
    ...
    fun dismiss() {
        isShowing = false
        MessageBox.queue.lastMessageBoxInstance = null;
        queue.showNext()
        enableAddLock = 3
    ...
    fun showNow(message: String?, buttons: List<String>?) {
        ...
        if (style === WindowsStyle.BlackNoFrame) {
            ...
            dialog = WhiteBottom(context, R.style.dialog)
        }else if (style === WindowsStyle.WhiteMiddle) {
            dialog = WhiteMiddle(context, R.style.dialog)
        }else if (style === WindowsStyle.WhiteBottomNoTitle) {
            dialog = WhiteBottomNoTitle(context, R.style.dialog)
        }
        //强制Dialogqia
        val windowManager = A.a().getWindowManager()
        ...
        queue.lastMessageBoxInstance = this;
        isShowing = true
    ...
    fun setListener(func: (Int) -> Unit): MessageBox {
